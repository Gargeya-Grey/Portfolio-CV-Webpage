    ...
    return (
        <div className="fixed bottom-6 sm:bottom-8 left-0 right-0 z-50 flex justify-center px-4 pointer-events-none">
            <motion.nav
                ...
                className="pointer-events-auto flex items-center gap-0 rounded-full bg-white/70 px-1.5 py-1.5 shadow-lg shadow-zinc-500/5 backdrop-blur-xl border border-white/20"
            >
                {navItems.map((item) => (
                    <Link
                        key={item.name}
                        href={item.href}
                        className="group relative px-2.5 sm:px-5 py-2 sm:py-2.5 text-[10px] sm:text-sm font-medium text-zinc-600 transition-colors hover:text-zinc-900"
                    >
